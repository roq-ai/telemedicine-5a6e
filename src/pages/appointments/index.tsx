...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAppointments({
        relations: ['user_appointment_doctor_idTouser', 'user_appointment_customer_idTouser'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['location.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      accessorKey: 'time',
      cell: ({ row: { original: record } }: any) =>
        record?.time ? format(parseISO(record?.time as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'location', header: 'Location', accessorKey: 'location' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_doctor_idTouser',
          header: 'User Appointment Doctor Id Touser',
          accessorKey: 'user_appointment_doctor_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_doctor_idTouser?.id}`}
            >
              {record.user_appointment_doctor_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_customer_idTouser',
          header: 'User Appointment Customer Id Touser',
          accessorKey: 'user_appointment_customer_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_customer_idTouser?.id}`}
            >
              {record.user_appointment_customer_idTouser?.email}
            </Link>
          ),
        }
      : null,
