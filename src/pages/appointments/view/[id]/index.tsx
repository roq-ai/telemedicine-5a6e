...
import { getAppointmentById } from 'apiSdk/appointments';
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Date"
                  text={data?.date ? format(parseISO(data?.date as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Time"
                  text={data?.time ? format(parseISO(data?.time as unknown as string), 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Appointment Customer Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_appointment_customer_idTouser?.id}`}>
                        {data?.user_appointment_customer_idTouser?.email}
                      </Link>
                    }
                  />
                )}
