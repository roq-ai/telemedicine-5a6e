...
import { getMedicalRecords } from 'apiSdk/medical-records';
...
import { AppointmentListPage as ShowCaseList } from 'pages/appointments';

function DashboardPage() {
  const { session } = useSession();

  const medicalRecordStat = useSWR<PaginatedInterface<MedicalRecordInterface>>(
    () => `/medical-records`,
    () => getMedicalRecords({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: medicalRecordStat?.data?.totalCount,
      isLoading: medicalRecordStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`appointments`} passHref legacyBehavior>
                  <Button
